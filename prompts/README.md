...
A checkpoint is not complete until its required package files exist, every referenced ledger ID resolves, integrity checks pass, and a PR exists when GitHub write access is available. Failed captures should be reported as incomplete rather than cleaned up retroactively.
